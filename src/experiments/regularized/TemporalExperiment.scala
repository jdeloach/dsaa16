...
object TemporalTest {
  ...
  def supervisedTests(folds: Array[(RDD[LabeledPoint],RDD[LabeledPoint])], noiseLevel: Double) {
    ...
    val naiveBayesPRC = folds.map{ case (train,test) => {
      ...
      val futureAcc = mislabeled.filter { x => model.predict(x.features) == x.asInstanceOf[MislabeledLabeledPoint].realLabel }.count / mislabeled.count.toDouble
      
      (new BinaryClassificationMetrics(predsAndLabel2015, 100).areaUnderPR,new BinaryClassificationMetrics(predsAndLabel2016, 100).areaUnderPR, futureAcc)
    }}.unzip3
    ...
    diagnostics(s"NaiveBayes -- Noise Level: $noiseLevel, Test 2015 auPRC: ${naiveBayesPRC._1.sum / folds.size.toDouble} Test 2016 auPRC: ${naiveBayesPRC._2.sum / folds.size.toDouble}  FUTURE PRED ACC: ${naiveBayesPRC._3.sum / folds.size.toDouble}")
    diagnostics(s"LogisticRegression -- Noise Level: $noiseLevel, Test 2015 auPRC: ${lrPRC._1.sum / folds.size.toDouble} Test 2016 auPRC: ${lrPRC._2.sum / folds.size.toDouble} FUTURE PRED ACC: ${lrPRC._3.sum / folds.size.toDouble}")
    diagnostics(s"SVM -- Noise Level: $noiseLevel, Test 2015 auPRC: ${svmPRC._1.sum / folds.size.toDouble} Test 2016 auPRC: ${svmPRC._2.sum / folds.size.toDouble} FUTURE PRED ACC: ${svmPRC._3.sum / folds.size.toDouble}")
  }
  ...
  def lrLr(folds: Array[(RDD[LabeledPoint],RDD[LabeledPoint])], noiseLevel: Double) {
    ...
    pTildes.foreach{ pTilde =>
      lambdaUs.foreach { lambdaU =>
        ...
        diagnostics(s"Noise Level: $noiseLevel, pTilde: " + pTilde + ", lambdaU: " + lambdaU + " Test2015 auPRC for LR: " + (lrAvgAuPRC._1.sum / folds.size.toDouble) + " Test 2016 auPRC: " + (lrAvgAuPRC._2.sum / folds.size.toDouble) + s"FUTURE PRED ACC: ${lrAvgAuPRC._3.sum / folds.size.toDouble}")   
      }
    }
  }
  ...
  def experiment(train: RDD[LabeledPoint], test: RDD[LabeledPoint], pTilde: Double, lambdaU: Double) : (Double,Double,Double) = {
      ...
      val mislabeled = test.filter { x => val y = x.asInstanceOf[MislabeledLabeledPoint]; y.realLabel != y.label }
      val futureAcc = mislabeled.filter { x => LRmodel.predict(x.features) == x.asInstanceOf[MislabeledLabeledPoint].realLabel }.count / mislabeled.count.toDouble
